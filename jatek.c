...
void
Hang (char *s)			// külsõ hangfájl lejátszása (csak példaként, nem használjuk)
{
  char buf[50];
...
main (int argc, char *argv[])
{
  ...
  do
    {				/* EZ MEGY KÖRBE-KÖRBE JÁTÉKONKÉNT */
      Opsonsz_aktiv ();
      if (MUZIK == 1)
	r = fopen ("/dev/audio", "w");
	...
	sprintf (buf, "%s", M_GepKezd ());	// kezdõ feliratok
...
/* Fõhurok */
      ...
      do
	{
	  ...
	  if (!kezdes)		/* a kezdõlépést már kezeltük korábban */
		...
		sprintf (buf, "%s", M_GepLep (random () % 21 - 11));
	  ...
	  for (i = 0; i < 10; ++i)
		...
		if (MUZIK == 1)
		  fwrite (pingpong[L - 1], 1, 8192, r);	/* Klatty! */

		Tabla[i][TJ[i] - 1] = L;	// Meglépjük a lépést az adminisztrációs táblán is.
		fl_deactivate_object (rubrika[i][TJ[i] - 1]);	// Leszereljük a már
		// "leszerepelt" mezõt.
		if (TJ[i] > 1)
		  fl_activate_object (rubrika[i][TJ[i] - 2]);
		else
		  +telioszlopok;	// Ha van még az aktuális oszlopban üres rubrika,
